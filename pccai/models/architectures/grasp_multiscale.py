...
class GeoResCompressionMultiscale(nn.Module):
    """
    Geometric Residual Analysis and Synthesis for PCC
    """

    def __init__(self, net_config, syntax):
        super(GeoResCompressionMultiscale, self).__init__()
        ...
        net_config['res_dec']['dims'][0] = net_config['vox_dec']['dims'][-1] + net_config['neighbor_enc']['fc_dims'][-1]
        ...
        self.neighbor_enc = PointNet(net_config['neighbor_enc'])
    ...
    def forward(self, coords):
        ...
        x_neighbor = torch.zeros((x_coarse_deq.shape[0], self.fit_num, 3), device = x_coarse_deq.device)
        batch_size = x.C[-1][0].item() + 1
        tot = 0
        with torch.no_grad():
            for pc_cnt in range(batch_size):
                x_coarse_cur = (x_coarse.C[x_coarse.C[:, 0] == pc_cnt][:, 1:]).float().contiguous()
                if self.faiss_resource == None:
                    self.faiss_resource = faiss.StandardGpuResources()
                    self.faiss_gpu_index_flat = faiss.GpuIndexFlatL2(self.faiss_resource, 3)
                self.faiss_gpu_index_flat.add(x_coarse_cur)
                _, I = self.faiss_gpu_index_flat.search(x_coarse_cur, self.fit_num)
                self.faiss_gpu_index_flat.reset()
                x_coarse_rep = x_coarse_cur.unsqueeze(1).repeat_interleave(self.fit_num, dim=1)
                x_neighbor[tot:tot+x_coarse_cur.shape[0], :, :] = x_coarse_cur[I] - x_coarse_rep
                mask = torch.logical_or(
                    torch.max(x_neighbor, dim=2)[0] > self.fit_radius,
                    torch.min(x_neighbor, dim=2)[0] < self.fit_radius
                )
                I[mask] = I[:,0].unsqueeze(-1).repeat_interleave(self.fit_num, dim=1)[mask]
                x_neighbor[tot : tot + x_coarse_cur.shape[0], :, :][mask] = x_coarse_cur[I[mask]] - x_coarse_rep[mask] # recompute the outlier distance
                tot += x_coarse_cur.shape[0]*self.point_mul

        x_neighbor_feat = self.neighbor_enc(x_neighbor)
        res.F = torch.cat([res.F, x_neighbor_feat], dim=1)
